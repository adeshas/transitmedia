 ...
 * Page class
 */
class MainBusesPreview extends MainBuses
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MainBusesPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "main_campaigns") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=main_campaigns&" . GetForeignKeyUrl("fk_id", $this->exterior_campaign_id->QueryStringValue) . "&" . GetForeignKeyUrl("fk_id", $this->interior_campaign_id->QueryStringValue) . "";
        }
        return $url;
    }
        ...
        if ($masterTblVar == "main_campaigns") {
            ...
            if ($x !== false) {
                ...
                $val = $this->unquoteValue($val, "DB");
                 $this->exterior_campaign_id->setQueryStringValue($val);
            }
            $find = "\"interior_campaign_id\"=";
            $x = strpos($f, $find);
            if ($x !== false) {
                $x += strlen($find);
                $val = substr($f, $x);
                $val = $this->unquoteValue($val, "DB");
                 $this->interior_campaign_id->setQueryStringValue($val);
            }
        }
    }
