...
/**
 * Table class for print_orders
 */
class PrintOrders extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'print_orders';
        $this->TableName = 'print_orders';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "\"print_orders\"";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id
        $this->id = new DbField('print_orders', 'print_orders', 'x_id', 'id', '"id"', 'CAST("id" AS varchar(255))', 3, 4, -1, false, '"id"', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // campaign_id
        $this->campaign_id = new DbField('print_orders', 'print_orders', 'x_campaign_id', 'campaign_id', '"campaign_id"', 'CAST("campaign_id" AS varchar(255))', 3, 4, -1, false, '"campaign_id"', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // printer_id
        $this->printer_id = new DbField('print_orders', 'print_orders', 'x_printer_id', 'printer_id', '"printer_id"', 'CAST("printer_id" AS varchar(255))', 3, 4, -1, false, '"printer_id"', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->printer_id->Lookup = new Lookup('printer_id', 'printers', false, 'id', ["name","","",""], [], [], [], [], [], [], '"id" DESC', '');
        $this->printer_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['printer_id'] = &$this->printer_id;

        // ts_created
        $this->ts_created = new DbField('print_orders', 'print_orders', 'x_ts_created', 'ts_created', '"ts_created"', CastDateFieldForLike("\"ts_created\"", 0, "DB"), 135, 8, 0, false, '"ts_created"', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // link
        $this->link = new DbField('print_orders', 'print_orders', 'x_link', 'link', '"link"', '"link"', 201, 0, -1, false, '"link"', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->link->Sortable = true; // Allow sort
        $this->Fields['link'] = &$this->link;

        // quantity
        $this->quantity = new DbField('print_orders', 'print_orders', 'x_quantity', 'quantity', '"quantity"', 'CAST("quantity" AS varchar(255))', 3, 4, -1, false, '"quantity"', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // tags
        $this->tags = new DbField('print_orders', 'print_orders', 'x_tags', 'tags', '((select name from main_campaigns where id = campaign_id) || \' | \' || (select name from printers where id = printer_id))', '((select name from main_campaigns where id = campaign_id) || \' | \' || (select name from printers where id = printer_id))', 201, 0, -1, false, '((select name from main_campaigns where id = campaign_id) || \' | \' || (select name from printers where id = printer_id))', false, false, false, 'FORMATTED TEXT', 'TEXTAREA');
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "\"print_orders\"";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->id->setDbValue($conn->fetchColumn("SELECT currval('print_orders_id_seq'::regclass)"));
    ...
    {
        global $Language;
        if ($pageName == "printordersview") {
            return $Language->phrase("View");
        } elseif ($pageName == "printordersedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "printordersadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            ...
            case Config("API_ADD_ACTION"):
                return "PrintOrdersAdd";
            case Config("API_EDIT_ACTION"):
                return "PrintOrdersEdit";
            case Config("API_DELETE_ACTION"):
                return "PrintOrdersDelete";
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("printordersview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("printordersview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "printordersadd?" . $this->getUrlParm($parm);
        } else {
            $url = "printordersadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        $url = $this->keyUrl("printordersedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("printordersadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("printordersdelete", $this->getUrlParm());
    }
