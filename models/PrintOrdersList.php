 ...
 * Page class
 */
class PrintOrdersList extends PrintOrders
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'print_orders';

    // Page object name
    public $PageObjName = "PrintOrdersList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fprint_orderslist";
    ...
    public function __construct()
    {
        ...
        // Table object (print_orders)
        if (!isset($GLOBALS["print_orders"]) || get_class($GLOBALS["print_orders"]) == PROJECT_NAMESPACE . "print_orders") {
            $GLOBALS["print_orders"] = &$this;
        }
        ...
        $this->AddUrl = "printordersadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "printordersdelete";
        $this->MultiUpdateUrl = "printordersupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'print_orders');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fprint_orderslistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("print_orders"));
                $doc->Text = @$content;
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->printer_id->AdvancedSearch->toJson(), ","); // Field printer_id
        ...
        $filterList = Concat($filterList, $this->all_codes_assigned_in_campaign->AdvancedSearch->toJson(), ","); // Field all_codes_assigned_in_campaign
        $filterList = Concat($filterList, $this->bus_codes->AdvancedSearch->toJson(), ","); // Field bus_codes
        $filterList = Concat($filterList, $this->available_codes_to_be_assigned->AdvancedSearch->toJson(), ","); // Field available_codes_to_be_assigned
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fprint_orderslistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field printer_id
        $this->printer_id->AdvancedSearch->SearchValue = @$filter["x_printer_id"];
        $this->printer_id->AdvancedSearch->SearchOperator = @$filter["z_printer_id"];
        $this->printer_id->AdvancedSearch->SearchCondition = @$filter["v_printer_id"];
        $this->printer_id->AdvancedSearch->SearchValue2 = @$filter["y_printer_id"];
        $this->printer_id->AdvancedSearch->SearchOperator2 = @$filter["w_printer_id"];
        $this->printer_id->AdvancedSearch->save();
        ...
        // Field all_codes_assigned_in_campaign
        $this->all_codes_assigned_in_campaign->AdvancedSearch->SearchValue = @$filter["x_all_codes_assigned_in_campaign"];
        $this->all_codes_assigned_in_campaign->AdvancedSearch->SearchOperator = @$filter["z_all_codes_assigned_in_campaign"];
        $this->all_codes_assigned_in_campaign->AdvancedSearch->SearchCondition = @$filter["v_all_codes_assigned_in_campaign"];
        $this->all_codes_assigned_in_campaign->AdvancedSearch->SearchValue2 = @$filter["y_all_codes_assigned_in_campaign"];
        $this->all_codes_assigned_in_campaign->AdvancedSearch->SearchOperator2 = @$filter["w_all_codes_assigned_in_campaign"];
        $this->all_codes_assigned_in_campaign->AdvancedSearch->save();
        ...
        // Field available_codes_to_be_assigned
        $this->available_codes_to_be_assigned->AdvancedSearch->SearchValue = @$filter["x_available_codes_to_be_assigned"];
        $this->available_codes_to_be_assigned->AdvancedSearch->SearchOperator = @$filter["z_available_codes_to_be_assigned"];
        $this->available_codes_to_be_assigned->AdvancedSearch->SearchCondition = @$filter["v_available_codes_to_be_assigned"];
        $this->available_codes_to_be_assigned->AdvancedSearch->SearchValue2 = @$filter["y_available_codes_to_be_assigned"];
        $this->available_codes_to_be_assigned->AdvancedSearch->SearchOperator2 = @$filter["w_available_codes_to_be_assigned"];
        $this->available_codes_to_be_assigned->AdvancedSearch->save();
    ...
    protected function basicSearchSql($arKeywords, $type)
    {
        ...
        $this->buildBasicSearchSql($where, $this->all_codes_assigned_in_campaign, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->bus_codes, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->available_codes_to_be_assigned, $arKeywords, $type);
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->printer_id); // printer_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->printer_id->setSort("");
                $this->ts_created->setSort("");
                $this->link->setSort("");
                $this->quantity->setSort("");
                $this->approved->setSort("");
                $this->comments->setSort("");
                $this->all_codes_assigned_in_campaign->setSort("");
                $this->bus_codes->setSort("");
                $this->available_codes_to_be_assigned->setSort("");
                $this->tags->setSort("");
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fprint_orderslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fprint_orderslistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fprint_orderslist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fprint_orderslist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fprint_orderslist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fprint_orderslist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_print_orders" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_print_orders\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fprint_orderslist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fprint_orderslistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
