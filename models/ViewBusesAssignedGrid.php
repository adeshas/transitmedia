 ...
 * Page class
 */
class ViewBusesAssignedGrid extends ViewBusesAssigned
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewBusesAssignedGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fview_buses_assignedgrid";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_bus") && $CurrentForm->hasValue("o_bus") && $this->bus->CurrentValue != $this->bus->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_transaction_id") && $CurrentForm->hasValue("o_transaction_id") && $this->transaction_id->CurrentValue != $this->transaction_id->OldValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->bus->CurrentValue = null;
        $this->bus->OldValue = $this->bus->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'bus' first before field var 'x_bus'
        $val = $CurrentForm->hasValue("bus") ? $CurrentForm->getValue("bus") : $CurrentForm->getValue("x_bus");
        if (!$this->bus->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->bus->Visible = false; // Disable update for API request
            } else {
                $this->bus->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_bus")) {
            $this->bus->setOldValue($CurrentForm->getValue("o_bus"));
        }
        ...
        if ($CurrentForm->hasValue("o_transaction_id")) {
            $this->transaction_id->setOldValue($CurrentForm->getValue("o_transaction_id"));
        }
    }
    ...
    {
        global $CurrentForm;
        $this->bus->CurrentValue = $this->bus->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['bus'] = $this->bus->CurrentValue;
        $row['transaction_id'] = $this->transaction_id->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // bus
            $this->bus->EditAttrs["class"] = "form-control";
            $this->bus->EditCustomAttributes = "";
            $this->bus->EditValue = HtmlEncode($this->bus->CurrentValue);
            $this->bus->PlaceHolder = RemoveHtml($this->bus->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->bus->Required) {
            if (!$this->bus->IsDetailKey && EmptyValue($this->bus->FormValue)) {
                $this->bus->addErrorMessage(str_replace("%s", $this->bus->caption(), $this->bus->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "view_campaigns_pending") {
        ...
        // bus
        $this->bus->setDbValueDef($rsnew, $this->bus->CurrentValue, null, false);
