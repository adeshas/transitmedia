 ...
 * Page class
 */
class ViewCampaignsCompletedSearch extends ViewCampaignsCompleted
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewCampaignsCompletedSearch";
        ...
        if ($this->isPageRequest()) {
            ...
            if ($this->isSearch()) {
                ...
                if ($srchStr != "") {
                    ...
                    $srchStr = "viewcampaignscompletedlist" . "?" . $srchStr;
    ...
    protected function buildAdvancedSearch()
    {
        ...
        $this->buildSearchUrl($srchUrl, $this->agency_fee); // agency_fee
        $this->buildSearchUrl($srchUrl, $this->lasaa_fee); // lasaa_fee
        $this->buildSearchUrl($srchUrl, $this->printers_fee); // printers_fee
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->agency_fee->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->lasaa_fee->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->printers_fee->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->transaction_status->AdvancedSearch->post()) {
            $hasValue = true;
        }
        if ($this->payment_status->AdvancedSearch->post()) {
            $hasValue = true;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->isExport()) {
                ...
                $this->inventory->LinkAttrs["data-tooltip-id"] = "tt_view_campaigns_completed_x_inventory";
            ...
            $this->payment_date->EditAttrs["class"] = "form-control";
            $this->payment_date->EditCustomAttributes = "";
            $this->payment_date->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->payment_date->AdvancedSearch->SearchValue2, 0), 8));
            $this->payment_date->PlaceHolder = RemoveHtml($this->payment_date->caption());
            ...
            if (!$this->payment_status->Raw) {
                $this->payment_status->AdvancedSearch->SearchValue = HtmlDecode($this->payment_status->AdvancedSearch->SearchValue);
            }
            $this->payment_status->EditValue = HtmlEncode($this->payment_status->AdvancedSearch->SearchValue);
            ...
            if ($this->status_id->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "\"id\"" . SearchString("=", $this->status_id->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->payment_date->AdvancedSearch->SearchValue2)) {
            $this->payment_date->addErrorMessage($this->payment_date->getErrorMessage(false));
        }
        ...
        if (!CheckInteger($this->agency_fee->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckInteger($this->printers_fee->AdvancedSearch->SearchValue)) {
        ...
        if (!CheckInteger($this->payment_status_id->AdvancedSearch->SearchValue)) {
            $this->payment_status_id->addErrorMessage($this->payment_status_id->getErrorMessage(false));
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("viewcampaignscompletedlist"), "", $this->TableVar, true);
        $pageId = "search";
