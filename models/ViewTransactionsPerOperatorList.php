 ...
 * Page class
 */
class ViewTransactionsPerOperatorList extends ViewTransactionsPerOperator
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'view_transactions_per_operator';

    // Page object name
    public $PageObjName = "ViewTransactionsPerOperatorList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fview_transactions_per_operatorlist";
    ...
    public function __construct()
    {
        ...
        // Table object (view_transactions_per_operator)
        if (!isset($GLOBALS["view_transactions_per_operator"]) || get_class($GLOBALS["view_transactions_per_operator"]) == PROJECT_NAMESPACE . "view_transactions_per_operator") {
            $GLOBALS["view_transactions_per_operator"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_transactions_per_operator');
        }
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fview_transactions_per_operatorlistsrch";
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("view_transactions_per_operator"));
                $doc->Text = @$content;
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fview_transactions_per_operatorlistsrch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_transactions_per_operatorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_transactions_per_operatorlistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fview_transactions_per_operatorlist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (!$this->isExport()) {
                ...
                $this->inventory->LinkAttrs["data-tooltip-id"] = "tt_view_transactions_per_operator_x" . $this->RowCount . "_inventory";
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fview_transactions_per_operatorlistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    protected function showOptionLink($id = "")
    {
        ...
        if ($Security->isLoggedIn() && !$Security->isAdmin() && !$this->userIDAllow($id)) {
            return $Security->isValidUserID($this->vendor_search_id->CurrentValue);
        }
        return true;
    }
