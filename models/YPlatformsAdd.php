 ...
 * Page class
 */
class YPlatformsAdd extends YPlatforms
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "YPlatformsAdd";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("y_operators", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("MainCampaignsGrid");
            if (in_array("main_campaigns", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->platform_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "main_campaigns"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->platform_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("YOperatorsGrid");
            if (in_array("y_operators", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->platform_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "y_operators"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->platform_id->setSessionValue(""); // Clear master key if insert failed
