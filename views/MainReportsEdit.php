...
// Page object
$MainReportsEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fmain_reportsedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fmain_reportsedit = currentForm = new ew.Form("fmain_reportsedit", "edit");
    ...
    fmain_reportsedit.addFields([
        ...
        ["ts_created", [fields.ts_created.visible && fields.ts_created.required ? ew.Validators.required(fields.ts_created.caption) : null], fields.ts_created.isInvalid],
    ...
    // Validate form
    fmain_reportsedit.validate = function () {
    ...
    // Form_CustomValidate
    fmain_reportsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmain_reportsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fmain_reportsedit.lists.type_id = <?= $Page->type_id->toClientList($Page) ?>;
    fmain_reportsedit.lists.campaign_id = <?= $Page->campaign_id->toClientList($Page) ?>;
    fmain_reportsedit.lists.ref_bus_id = <?= $Page->ref_bus_id->toClientList($Page) ?>;
    loadjs.done("fmain_reportsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmain_reportsedit" id="fmain_reportsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_main_reports_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="main_reports" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_main_reports_id">
<span<?= $Page->id->viewAttributes() ?>>
...
<input type="hidden" data-table="main_reports" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fmain_reportsedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fmain_reportsedit", "x_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fmain_reportsedit"], function() {
    fmain_reportsedit.createAutoSuggest(Object.assign({"id":"x_ref_bus_id","forceSelect":false}, ew.vars.tables.main_reports.fields.ref_bus_id.autoSuggestOptions));
});
</script>
...
<?php if ($Page->ts_created->Visible) { // ts_created ?>
    <div id="r_ts_created" class="form-group row">
        <label id="elh_main_reports_ts_created" for="x_ts_created" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ts_created->caption() ?><?= $Page->ts_created->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->ts_created->getDisplayValue($Page->ts_created->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="main_reports" data-field="x_ts_created" data-hidden="1" name="x_ts_created" id="x_ts_created" value="<?= HtmlEncode($Page->ts_created->CurrentValue) ?>">
<?php } else { ?>
<span id="el_main_reports_ts_created">
<span<?= $Page->ts_created->viewAttributes() ?>>
...
<input type="hidden" data-table="main_reports" data-field="x_ts_created" data-hidden="1" name="x_ts_created" id="x_ts_created" value="<?= HtmlEncode($Page->ts_created->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
