...
<?php if ($Page->_password->Visible) { // password ?>
    <div id="r_c__password" class="form-group row">
        <label id="elh_c_main_users__password" for="c__password" class="<?= $Page->LeftColumnClass ?>"><?= $Language->phrase("Confirm") ?> <?= $Page->_password->caption() ?><?= $Page->_password->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->_password->cellAttributes() ?>>
<?php if (!$Page->isConfirm()) { ?>
<span id="el_c_main_users__password">
<div class="input-group">
    <input type="password" name="c__password" id="c__password" autocomplete="new-password" data-field="x__password" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
    <div class="input-group-append"><button type="button" class="btn btn-default ew-toggle-password rounded-right" onclick="ew.togglePassword(event);"><i class="fas fa-eye"></i></button></div>
</div>
<?= $Page->_password->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->_password->getErrorMessage() ?></div>
...
<input type="hidden" data-table="main_users" data-field="x__password" data-hidden="1" name="c__password" id="c__password" value="<?= HtmlEncode($Page->_password->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
