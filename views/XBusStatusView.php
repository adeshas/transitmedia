...
<?php } ?>
<script>
if (!ew.vars.tables.x_bus_status) ew.vars.tables.x_bus_status = <?= JsonEncode(GetClientVar("tables", "x_bus_status")) ?>;
</script>
<?php if (!$Page->isExport()) { ?>
...
$Page->showMessage();
?>
<form name="fx_bus_statusview" id="fx_bus_statusview" class="form-inline ew-form ew-view-form" action="<?= CurrentPageUrl(false) ?>" method="post">
